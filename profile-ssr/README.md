...
| Arquivo | Pra quê |
|---|---|
| `code-patterns.md` | Estrutura de pastas, schemas Zod, services, repository, mapper toOutput |
| `api-patterns.md` | API routes Next.js, validação, respostas, erros |
| `ui-patterns.md` | Componentes React, formulários, Tailwind, **responsividade mobile** |
| `claude-md.extension.md` | Seções específicas pra SSR (Next.js, deploy, comandos) |
...
- **tradeBot** (`tradebot-202605`) — atualizações importadas:
  - Princípios LLM-first (em `common/`)
  - Critérios meta M1/M2/M3 (em `common/`)
  - `_open-issues.md` template (em `common/`)
  - Adições de responsividade mobile e integração com skill
    `ui-design-system` (em `ui-patterns.md` deste perfil)

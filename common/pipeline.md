...
discovery, os agentes de conceituação/documentação/spec ([[agents/README|agents/]]),
o protocolo de gates e o ciclo Cowork↔Claude Code do [[spec-guide]].
...
**1 · Conceituação** — [[agente-conceituacao]]. Dor→conceito: diagrama de
...
**2 · Documentação funcional** — [[agente-documentacao-funcional]]. Como a
...
**3 · Geração de spec** — [[agente-gerador-spec]]. Corta o incremento em N
specs verticais no template do [[spec-guide]], redistribuindo os artefatos
a montante. Gate de corte (alto risco). Saída: `specs/{domínio}/NN.md` +
[[_overview]] + [[_open-issues]].

**4 · Implementação** — Claude Code, **uma spec por sessão**: lê
[[claude-md.template|CLAUDE.md]] → a spec → os `docs/` referenciados →
implementa → reporta (critérios ✅/❌ + decisões). Detalhe do ciclo e das
regras operacionais no [[spec-guide]].
...
[[protocolo-de-gates]] é a **fonte canônica** de gate e calibragem por
...
meta **M1-M3** do [[spec-guide]] são a **aplicação** desse princípio à
...
- **Mudança pequena / projeto pequeno:** entrar direto no trecho
  ...
  [[spec-guide]] cobre esse caminho (inclui "fixes vs specs").
...
| Gates | Transversal — [[protocolo-de-gates]] |
